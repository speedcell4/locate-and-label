...
class Identifier(BertPreTrainedModel):
    ...
    def __init__(self, config: BertConfig, embed: torch.tensor, cls_token: int, entity_types: int,
                 size_embedding: int, prop_drop: float, freeze_transformer: bool, lstm_layers: int = 1,
                 lstm_drop: float = 0.4, pos_size: int = 25, char_lstm_layers: int = 1, char_lstm_drop: int = 0.2,
                 char_size: int = 25, use_glove: bool = True, use_pos: bool = True, use_char_lstm: bool = True,
                 spn_filter: int = 5, pool_type: str = "max", use_entity_ctx=False, use_size_embedding=False,
                 reduce_dim=False, bert_before_lstm=False, no_filter=True, no_regressor=True, norm="sigmoid",
    ...
    def _common_forward(self, encodings: torch.tensor, context_masks: torch.tensor, token_masks: torch.tensor,
                        entity_masks: torch.tensor,
                        token_masks_bool: torch.tensor, entity_masks_token: torch.tensor,
                        entity_spans_token: torch.tensor,
                        entity_sizes: torch.tensor, pos_encoding: torch.tensor = None,
        ...
        bin_clf, entity_spans_pool, offsets1 = self._spn(encodings, h, h_token, entity_masks, entity_masks_token,
                                                         size_embeddings, entity_spans_token, token_count,
                                                         token_masks_bool)
        ...
        # entity_spans_token
        entity_spans_token[entity_spans_token[:, :, 0] >= entity_spans_token[:, :, 1]] = old[
        ...
        if self.use_size_embedding:
            ...
            size_embeddings = self.size_embeddings(entity_sizes)  # embed entity candidate sizes
        entity_clf, entity_spans_pool, offsets2 = self._classify_entities(encodings, h, h_token, entity_masks,
                                                                          entity_masks_token, size_embeddings,
                                                                          entity_spans_token, token_count,
                                                                          token_masks_bool)
    ...
    def _forward_train(self, encodings: torch.tensor, context_masks: torch.tensor, token_masks: torch.tensor,
                       ...
                       entity_spans_token: torch.tensor,
                       entity_sizes: torch.tensor, pos_encoding: torch.tensor = None,
                       wordvec_encoding: torch.tensor = None, char_encoding: torch.tensor = None, token_masks_char=None,
        ...
        return self._common_forward(encodings, context_masks, token_masks, entity_masks,
                                    ...
                                    entity_sizes, pos_encoding, wordvec_encoding, char_encoding, token_masks_char,
                                    char_count)

    def _forward_eval(self, encodings: torch.tensor, context_masks: torch.tensor, token_masks: torch.tensor,
                      entity_masks: torch.tensor,
                      entity_masks_token: torch.tensor, entity_spans_token: torch.tensor, entity_spans: torch.tensor,
                      entity_sample_masks: torch.tensor,
                      entity_sizes: torch.tensor, token_masks_bool: torch.tensor, pos_encoding: torch.tensor = None,
                      wordvec_encoding=None, char_encoding: torch.tensor = None, token_masks_char: torch.tensor = None,
                      char_count: torch.tensor = None):
        entity_clf, offsets2, bin_clf, offsets1, spn_mask, illegal_mask = self._common_forward(encodings, context_masks,
                                                                                               token_masks,
                                                                                               entity_masks,
                                                                                               token_masks_bool,
                                                                                               entity_masks_token,
    ...
    def _spn(self, encodings, h, h_token, entity_masks, entity_masks_token, size_embeddings, entity_spans_token,
             token_count, token_masks_bool):
        # import pdb; pdb.set_trace()
        ...
        start_end_embedding_outer = start_end_embedding_outer.view(start_end_embedding_outer.size(0),
                                                                   start_end_embedding_outer.size(1), -1)
    ...
    def _classify_entities(self, encodings, h, h_token, entity_masks, entity_masks_token, size_embeddings,
                           entity_spans_token, token_count, token_masks_bool):
        # # max pool entity candidate spans
        # m = (entity_masks.unsqueeze(-1) == 0).float() * (-1e30)
        # entity_spans_pool = m + h.unsqueeze(1).repeat(1, entity_masks.shape[1], 1, 1)
        # entity_spans_pool = entity_spans_pool.max(dim=2)[0]
