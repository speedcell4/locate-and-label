...
class IdentifierTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        self._logger.info(f'Datasets: {train_path}, {valid_path}')
        self._logger.info(f'Model type: {args.model_type}')
        ...
        # read datasets
        input_reader = input_reader_cls(types_path, self._tokenizer, args.iou_spn, args.iou_classifier,
                                        args.neg_entity_count,
                                        args.neg_relation_count, args.window_size, self._logger,
                                        wordvec_filename=args.wordvec_path)
        ...
        if self.args.entity_loss == "celoss":
            ...
            entity_criterion = FocalLoss(class_num=input_reader.entity_type_count, reduction='none',
                                         gamma=self.args.entity_gamma)

        compute_loss = IdentifierLoss(filter_criterion, offset_criterion, giou_criterion, entity_criterion, model,
                                      optimizer, scheduler, args.iou_weight, args.max_grad_norm, args.iou_classifier,
                                      filter_weight=args.filter_loss_weight, offset_weight=args.offset_loss_weight,
                                      giou_weight=args.giou_loss_weight, entity_weight=args.entity_loss_weight,
                                      iou_gamma=args.iou_gamma)
        ...
        self._logger.info(f"Logged in: {self._log_path}")
        self._logger.info(f"Saved in: {self._save_path}")
    ...
    def eval(self, dataset_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        self._logger.info(f"Dataset: {dataset_path}")
        self._logger.info(f"Model: {args.model_type}")
        ...
        input_reader = input_reader_cls(types_path, self._tokenizer,
                                        window_size=args.window_size, logger=self._logger,
                                        wordvec_filename=args.wordvec_path)
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc=f'Train epoch {epoch}'):
            ...
            # forward step
            entity_logits, offsets2, bin_clf, offsets1, spn_mask, illegal_mask = model(encodings=batch['encodings'],
                                                                                       context_masks=batch[
                                                                                           'context_masks'],
                                                                                       entity_masks=batch[
                                                                                           'entity_masks'],
                                                                                       token_masks_bool=batch[
                                                                                           'token_masks_bool'],
                                                                                       entity_sizes=batch[
                                                                                           'entity_sizes'],
                                                                                       ...
                                                                                       pos_encoding=batch[
                                                                                           'pos_encoding'],
                                                                                       wordvec_encoding=batch[
                                                                                           'wordvec_encoding'],
                                                                                       char_encoding=batch[
                                                                                           'char_encoding'],
                                                                                       token_masks_char=batch[
                                                                                           'token_masks_char'],
                                                                                       char_count=batch['char_count'])

            # compute loss and optimize parameters
            batch_loss = compute_loss.compute(entity_logits=entity_logits, offsets1_pred=offsets1,
                                              offsets2_pred=offsets2, bin_logits=bin_clf, spn_mask=spn_mask,
                                              entity_types=batch['entity_types'],
                                              entity_types_1=batch['entity_types_1'], l_offsets_gt=batch['l_offsets'],
                                              r_offsets_gt=batch['r_offsets'],
                                              ious=batch['ious'], entity_sample_masks=batch['entity_sample_masks'],
                                              offset_sample_masks=batch['offset_sample_masks'],
                                              entity_spans_token=batch['entity_spans_token'], illegal_mask=illegal_mask)
    ...
    def _eval(self, model: torch.nn.Module, dataset: Dataset, input_reader: JsonInputReader,
        ...
        self._logger.info(f"Evaluate: {dataset.label}")
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader, total=total, desc=f'Evaluate epoch {epoch}'):
                ...
                result = model(encodings=batch['encodings'], context_masks=batch['context_masks'],
                               ...
                               token_masks=batch['token_masks'], entity_masks_token=batch['entity_masks_token'],
                               entity_spans_token=batch['entity_spans_token'], pos_encoding=batch['pos_encoding'],
    ...
    def _log_datasets(self, input_reader):
        self._logger.info(f'Relation type count: {input_reader.relation_type_count}')
        self._logger.info(f'Entity type count: {input_reader.entity_type_count}')
        ...
        for k, d in input_reader.datasets.items():
            self._logger.info(f'Dataset: {k}')
            self._logger.info(f'Document count: {d.document_count}')
            self._logger.info(f'Relation count: {d.relation_count}')
            self._logger.info(f'Entity count: {d.entity_count}')

        self._logger.info(f'Context size: {input_reader.context_size}')
