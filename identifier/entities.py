...
class Dataset(TorchDataset):
    ...
    def __init__(self, label, rel_types, entity_types, neg_entity_count,
                 neg_rel_count, window_size, iou_min, iou_max):
        self._label = label
    ...
    def create_document(self, tokens, entity_mentions, relations, doc_encoding, char_encoding) -> Document:
        document = Document(self._doc_id, tokens, entity_mentions, relations, doc_encoding, char_encoding)
    ...
    def __getitem__(self, index: int):
        ...
        if self._mode == Dataset.TRAIN_MODE:
            return sampling.create_train_sample(doc, self._neg_entity_count, self._neg_rel_count,
                                                self._window_size, len(self._rel_types), self.iou_min, self.iou_max)
        else:
            return sampling.create_eval_sample(doc, self._window_size)
